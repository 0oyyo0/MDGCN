...
def AssignLabels(useful_sp_lab, trlabels, telabels, trmask, temask): 
    ...
    for sp_idx in range(1, sp_num+1): #sp_num为949，就是为这949个像素块label
        pos1 = np.argwhere(useful_sp_lab==sp_idx) #useful_sp_lab的为label值得位置，这些位置的预测值
        if trmask[sp_idx-1, 0] == True: #trmask为true的地方指这些值是训练集
            pred_label = trlabels[sp_idx-1] # 这些位置的标签是训练集，不需要预测，我已经知道他是什么了。那么，他为什么会出错？
        else:
            pred_label = telabels[sp_idx-1] # testlabel是生成的？yes pixel_wise_pred
            # 那么就有一个问题，原来是train的位置的标签使用的是原来的标签，有可能错了啊
        # pred_label = telabels[sp_idx-1] #testlabel是生成的？yes pixel_wise_pred
...
def GetExcelData(gt, pred_labels, trpos):
    ...
    OA = 1-overall_err_num/np.shape(np.argwhere(gt>0))[0] #Overall accuracy总体精度是模型在所有测试集上预测正确的与总体数量之间的比值(正确分类的测试样本数 / 总测试样本数)
    AA = np.mean(per_acc) # average accuracy平均精度是每一类预测正确的与每一类总体数量之间的比值，最终再取每一类的精度的平均值(每个类的正确率的平均)
    # kappa
    ...
    for lab_idx in range(1, num_classes+1):
        ...
        ab1 += a1*b1
    Pe = ab1/(n*n)
    kappa_coef = (OA-Pe)/(1-Pe) #Kappa系数用于一致性检验，也可以用于衡量分类精度，kappa系数的计算基于混淆矩阵
